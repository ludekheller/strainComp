...
def genprojgrid(oris,gdata=None,nump=1001,proj='equalarea',method2='linear',gdout=False,poris=None):
    #project
    if poris is None:
        if proj=='stereo':
...
#        hist = np.log(hist)
    ...
    if kernel:
        ...
        oris=genori(dangle=5.0,hemi='both', tol=1e-2, rot=np.eye(3), half='no')
        xy = oris[0,:]**2 + oris[1,:]**2
        thtg = np.arctan2(np.sqrt(xy), oris[2,:])#elevation
        phig=np.arctan2(oris[1,:], oris[0,:])
        ...
        X,Y, hist, nummask, mask=genprojgrid02(oris[:,oris[2,:]>=0],gdata=H[oris[2,:]>=0],nump=1001,proj=proj,method2='linear',gdout=True)
        #if mrd:
    ...
    if Lim is not None:
        ...
        if Lim=='tri':
            ...
            grid_x,grid_y, nummask, mask=genprojgrid02(oristri,nump=bins,proj=proj,method2='linear',gdout=False)
            if kernel:
